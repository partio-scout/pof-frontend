...
function page_rewrite() {
    // This rule matches /aktiviteettihaku/{search clause}(/sivu/{page number}/)
    add_rewrite_rule( '^aktiviteettihaku/([^/]+)(/sivu/([^/]+))?(/)?$', 'index.php?s=$matches[1]&paged=$matches[2]', 'top' );
}
...
function translate_base_rewrite() {
    ...
    $wp_rewrite->search_base     = 'aktiviteettihaku';
}
